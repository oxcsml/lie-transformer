...
from eqv_transformer.train_tools import (log_tensorboard, parse_reports, print_reports, load_checkpoint,
                                         save_checkpoint, ExponentialMovingAverage)
...
flags.DEFINE_string('data_config', 'configs/constellation.py',
                    'Path to a data config file.')
flags.DEFINE_string('model_config', 'configs/set_transformer.py',
...
flags.DEFINE_integer('evaluate_every', 10000,
                     'Number of iterations between reporting validation loss.')
flags.DEFINE_integer('save_check_points', 100000, 'frequency with which to save checkpoints, in number of minibatches.')
...
def main():
    ...
    # Prepare environment
    results_folder_name = config.run_name + \
    ...
    model_opt = torch.optim.Adam(model_params, lr=opt_learning_rate,
    ...
    save_checkpoint(checkpoint_name, 0, model, model_opt, 0.)
        ...
        for batch_idx, data in enumerate(train_loader):
            ...
            if config.log_train_values:
                ...
                if batch_idx % config.report_loss_every == 0:
                    ...
                    print_reports(reports, start_t, epoch, batch_idx,
                                  len(train_loader.dataset) // config.batch_size, prefix='train')
            ...
            if batch_idx % config.evaluate_every == 0:
                ...
                print_reports(reports, start_t, epoch, batch_idx, len(train_loader.dataset) // config.batch_size,
                              prefix='test')

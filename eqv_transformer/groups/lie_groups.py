# Code from https://github.com/mfinzi/LieConv for using a subset of Lie groups

import torch
import numpy as np
from eqv_transformer.utils import export, Named
from eqv_transformer.groups.groups import Group
...
class LieGroup(Group):
    ...
    def representation_to_action(self, embedded_g):
        # Lie groups have the exponential map
        return self.exp(embedded_g)

    def log(self, u):
        raise NotImplementedError

    def action_to_representation(self, action):
        # Lie groups have the log map
    ...
    def components2matrix(self, a):
        """ Converts an element of the Lie algebra basis into a
        Lie algebra matrix
    ...
    def matrix2components(self, A):
        """ Converts a Lie algebra matirx to an element of the basis
        
        Parameters
        ----------
        A : torch.Tensor
            Tensor of shape (..., x_dim, x_dim)
        """
        raise NotImplementedError

    def inv(self, embedded_g):
        # In the Lie algebra the inverse is the negative.
        return -embedded_g

    def inv_matrix(self, g):
        # Shortcut using Lie group properties
    ...
    #         returns (a,v) with shapes [(*,n*nsamples,g_embed_dim),(*,n*nsamples,c)"""
    #     p, v, m = x
    #     expanded_a, expanded_q = self.lifted_elements(
    #         p, m, nsamples, **kwargs
...
def LieSubGroup(liegroup, generators):
    class subgroup(liegroup):
        def __init__(self, *args, **kwargs):
            ...
            self.orig_dim = self.g_g_embed_dim
            self.g_embed_dim = len(generators)
            self.q_embed_dim = self.orig_dim - len(generators)
...
class T(LieGroup):
    def __init__(self, k):
        ...
        super().__init__()
        self.q_embed_dim = 0
        self.x_dim = k  # dimension on which G acts
        self.g_embed_dim = k  # dimension that g is embedded into

    def lifted_elements(self, xyz, mask, nsamples, **kwargs):
        return xyz, None
...
class SO2(LieGroup):
    ...
    def components2matrix(self, a):  # a: (*,g_embed_dim)
        A = torch.zeros(*a.shape[:-1], 2, 2, device=a.device, dtype=a.dtype)
        A[..., 0, 1] = -a[..., 0]
        A[..., 1, 0] = a[..., 0]
        return A

    def matrix2components(self, A):  # A: (*,x_dim,x_dim)
...
class SO3(LieGroup):
    ...
    def lifted_elements(self, pt, mask, nsamples, **kwargs):
...
class SE3(SO3):
    ...
    def __init__(self, alpha=0.2, per_point=True):
        super().__init__()
        self.alpha = alpha
    ...
    #
    # def lifted_elements(self,pt,mask,nsamples,alpha=None):
    #     d=self.x_dim
    #     # Sample stabilizer of the origin
    #     #thetas = (torch.rand(*p.shape[:-1],num_samples).to(p.device)*2-1)*np.pi
    ...
    def lifted_elements(self, pt, mask, nsamples):
        """ pt (bs,n,D) mask (bs,n), per_point specifies whether to
            use a different group element per atom in the molecule"""
        # return farthest_lift(self,pt,mask,nsamples,alpha)
        # same lifts for each point right now
...
def farthest_lift(self, pt, mask, nsamples, alpha=0.5):
    nlift = 10
    bs, n = pt.shape[:2]
    d = self.g_embed_dim
    # Sample stabilizer of the origin
    ...
    picked_group_elems = torch.zeros(
        bs, n, nsamples, d, device=pt.device, dtype=pt.dtype
    )
    ...
    for i in range(nsamples):
        for j in np.random.permutation(n):
            ...
            # Get T(p)
            T = torch.zeros(
                *q.shape, 4, device=pt.device, dtype=pt.dtype
            )  # (bs,nlift,4,4)
            T[..., :, :] = torch.eye(4, device=pt.device, dtype=pt.dtype)
            ...
            distances = self.distance(
                picked_group_elems[..., None, :], a[:, None, None, :], alpha=alpha
            )  # (bs,n,nsamples,1,d)
            distances_m = torch.where(
                mask[:, :, None, None], distances, 1e7 * torch.ones_like(distances)
            )
            masked_distances = torch.where(
                ~picked_mask, distances_m, 1e8 * torch.ones_like(distances_m)
            )
            farthest_idx = (
                distances.min(dim=2)[0].min(dim=1)[0].argmax(-1)
            )  # (bs,n,nsamples,nlift) -> (bs,)
...
class Trivial(LieGroup):
    ...
    # def distance(self,abq_pairs):
    #     qa = abq_pairs[...,:self.q_embed_dim]
    #     qb = abq_pairs[...,self.q_embed_dim:]

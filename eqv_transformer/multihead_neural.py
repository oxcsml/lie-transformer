...
def MultiheadLinearBNact(n_heads, c_in, c_out, act="swish", bn=True):
...
def MultiheadWeightNet(in_dim, out_dim, act, bn, hid_dim=32):
    return nn.Sequential(
        *MultiheadLinearBNact(in_dim, hid_dim, act, bn),
        *MultiheadLinearBNact(hid_dim, hid_dim, act, bn),
        *MultiheadLinearBNact(hid_dim, out_dim, act, bn),
    )
